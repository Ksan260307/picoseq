...
# パートごとのピーク音量 (Q15)。
# 単独で鳴らした実測 RMS で釣り合いを取っている。ピーク値をそのまま音量と思うと外す:
# ベースは低音の倍音構成で実効エネルギーが大きく、伴奏は 2 段減衰で早く落ちる。
#   実測 (メロディ比): 旧 ベース 1.95x / リズム 0.99x / **伴奏 0.37x**
#   → 伴奏は 6 倍に増やした刻みが聞こえないので上げ、余った分はベースから回す。
# さらに全体を 15/16 に絞ってある。旧配分では 100 フレーズに 1 つ、
# 4 パートの立ち上がりが重なった瞬間に 16bit の上限へ達していた。
PEAKS = {
    WAVE_PULSE: 4608,      # 0.141
    WAVE_TRIANGLE: 7373,   # 0.225 (旧 0.30 — 実効エネルギーが大きいので下げる)
    WAVE_NOISE: 6144,      # 0.1875
    WAVE_SAW: 4915,        # 0.15  (旧 0.10 — 和音が薄かったので上げる)
}
